...
for i = 1:mouseMega.nMouse
    ...
    allTbl_ctxt{i} = allMouse{i}.behav;  allTbl_ctxt{i}.trialNum = (1:size(allMouse{i}.behav,1))';    allTbl_ctxt{i}.reactionTime(allTbl_ctxt{i}.reactionTime>=2.5) = nan; 
    ...
    preblockStart = (allMouse{i}.biasBlock.trans(blockStartFlag,1));
    disp(length(blockStart))
    blockBefIdx = cell(1,length(blockStart)); blockAftIdx = cell(1,length(blockStart));

    
    for j = 1:length(blockStart)
        [minV,preBlockIdx] = min(abs(allMouse{i}.behav.bias(preblockStart(j):blockStart(j))));
        preBlockIdx = preBlockIdx + preblockStart(j); blockBefIdx{j}  = (preBlockIdx-befAft/2+1):(preBlockIdx+befAft/2);

        [maxV,postBlockIdx] = max(abs(allMouse{i}.behav.bias(blockStart(j)+1:blockEnd(j))));
        postBlockIdx = postBlockIdx + blockStart(j); blockAftIdx{j}  = (postBlockIdx-befAft/2+1):(postBlockIdx+befAft/2);
        ...
        disp(['Unbiased = ' num2str(mean(allMouse{i}.behav.bias(preBlockIdx)))...
            ', biased = ' num2str(mean(allMouse{i}.behav.bias(postBlockIdx)))])
